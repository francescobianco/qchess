...
**Board rendering**: bitmap-only PNG via automatic Kitty/SIXEL backend selection. Kitty is used on kitty/WezTerm; SIXEL is selected on VTE/GNOME Terminal when detectable. There is no Unicode board fallback. If no bitmap-capable terminal is available and qchess cannot relaunch in kitty/WezTerm, the program exits before starting the TUI. Backends can be forced with `--graphics kitty` or `--graphics sixel`.

**PNG board placement**: Kitty bitmap placement uses a sub-cell pixel offset of `x=4`, `y=8`. The TUI coordinates are text centered inside terminal cells, while the PNG board is anchored to the top-left of the board cell grid. Terminal cells are taller than they are wide, so a 4 px horizontal and 8 px vertical offset makes the PNG square centers line up visually with the rank/file coordinate text.

**Board overlay behavior**: the TUI board area is only a black placeholder. The actual board is always a bitmap overlay. When menus or modals are open, the bitmap board is cleared and the board area remains black so overlays are not covered.

**Board cursor**: arrow keys move `App::selected_square` on the chessboard. In the PNG renderer the current square has a solid 2 px blue border drawn above the square and piece.

**Graphics terminal handoff**: in `--graphics auto`, if the current terminal does not support bitmap rendering and the app finds `kitty` or `wezterm` in `PATH`, it opens a separate window and relaunches qchess with `--graphics kitty`. The launched window uses explicit font size `14.0`. `QCHESS_GRAPHICS_CHILD=1` prevents recursive relaunches. If no graphics-capable terminal is found, qchess exits with an error.
...
**Theme**: current style from `docs/style.md` — general black background, white main text, black menu bar with white text, no bordered boxes around the board or move list, blue active square cursor, gray status bar with dark gray text. The engine analysis area uses a horizontal ASCII separator line with title, not a bordered box.

**Board geometry placeholder**: the TUI reserves a 34x17 board area: 1-column rank label strip, 8 squares at 4x2 terminal cells, one bottom coordinate bar, and a 1-column black separator on the right before the move list. This area is black unless the bitmap board is currently drawn.
